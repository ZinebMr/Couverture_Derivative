...
def run_allexec_tests(action):
    for exe in exec_folder.iterdir():
        ...
        #     continue
        if action == PRICE and not exe.match('**/price0*'):
            continue
        elif action == HEDGE and not exe.match('**/hedge*'):
            continue
...
def run_singleexec_test(exe_fullpath, action):
    ...
    for test_case in test_folder.iterdir():
        ...
        try:
            ...
            if type(exec_result) is not dict:
...
def run_compare_results(action):
    for output_folder in (outdir_folder / "Outputs").iterdir():
        ...
        if action == PRICE:
            ...
            if not output_folder.match('**/price0*'):
                continue
        elif action == HEDGE:
            ...
            if not output_folder.match('**/hedge*'):
                continue
